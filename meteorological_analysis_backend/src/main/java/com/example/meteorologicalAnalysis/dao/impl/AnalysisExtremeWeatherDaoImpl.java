...
public class AnalysisExtremeWeatherDaoImpl implements AnalysisExtremeWeatherDao {
    ...
    public List<Map<String, Object>> getExtremeTemperatures(int year) {
        String sql =
                ...
                "   FROM weather_analysis WHERE p_year = ? " +
                "), " +
                "max_temp_records AS ( " +
                "   SELECT w.data_time, w.month, w.day, w.hour, w.temperature " +
                "   FROM weather_analysis w " +
                "   JOIN temp_stats ts ON w.temperature = ts.max_temp " +
                "   WHERE w.p_year = ? ORDER BY w.data_time LIMIT 1 " +
                "), " +
                "min_temp_records AS ( " +
                "   SELECT w.data_time, w.month, w.day, w.hour, w.temperature " +
                "   FROM weather_analysis w " +
                ...
                "   WHERE w.p_year = ? ORDER BY w.data_time LIMIT 1 " +
                ") " +
                "SELECT type, temperature_value, time_str " +
                "FROM ( " +
                "   SELECT 'max' AS type, temperature AS temperature_value, " +
                "          CONCAT(month, '-', day, ' ', lpad(hour, 2, '0'), ':00') AS time_str " +
                "   FROM max_temp_records " +
                "   UNION ALL " +
                "   SELECT 'min' AS type, temperature AS temperature_value, " +
                "          CONCAT(month, '-', day, ' ', lpad(hour, 2, '0'), ':00') AS time_str " +
